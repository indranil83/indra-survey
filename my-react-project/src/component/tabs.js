...
const Tabs = () => {
  ...
  const courseDataArray = useSelector((state) => state.courses.courseDataArray);
  ...
  useEffect(() => {
    ...
    // alert()
    if (storedCourseData) {
      dispatch(setCourses(JSON.parse(storedCourseData)));  // Initialize Redux state with stored data
    }
  }, [dispatch]);
